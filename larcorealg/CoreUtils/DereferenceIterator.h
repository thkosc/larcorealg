  ...
  namespace util {
    ...
    namespace detail {
      ...
      public:
        ...
        DereferenceIteratorBase(Iter const& iter, initialize_tag) : iter(iter) {}

        /// Constructor: acquires the specified iterator
        DereferenceIteratorBase(Iter&& iter, initialize_tag) : iter(std::move(iter)) {}
        ...
        this_t operator+(difference_type offset) const { return {iter + offset}; }
        this_t operator-(difference_type offset) const { return {iter - offset}; }
        ...
        difference_type operator-(this_t const& other) const { return iter - other.iter; }
        ...
        bool operator==(this_t const& other) const { return other.iter == iter; }
        bool operator!=(this_t const& other) const { return other.iter != iter; }
        bool operator<=(this_t const& other) const { return other.iter <= iter; }
        bool operator>=(this_t const& other) const { return other.iter >= iter; }
        bool operator<(this_t const& other) const { return other.iter < iter; }
      ...
      template <typename Iter, typename Value>
      auto operator+(typename DereferenceIteratorBase<Iter, Value>::difference_type offset,
    ...
    using DereferenceIterator =
      detail::DereferenceIteratorBase<Iter, std::decay_t<decltype(**(std::declval<Iter>()))>>;
    ...
    {
      return detail::makeIteratorBox(beginDereferenceIterator(cont), endDereferenceIterator(cont));
    } // dereferenceIteratorLoop()
    ...
    {
      return detail::makeIteratorBox(rbeginDereferenceIterator(cont),
                                     rendDereferenceIterator(cont));
    ...
    {
      return detail::makeIteratorBox(cbeginDereferenceIterator(cont),
                                     cendDereferenceIterator(cont));
    ...
    {
      return detail::makeIteratorBox(crbeginDereferenceIterator(cont),
                                     crendDereferenceIterator(cont));
