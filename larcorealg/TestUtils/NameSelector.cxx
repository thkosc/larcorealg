...
namespace testing {
  ...
  void NameSelector::PrintConfiguration(std::ostream& out) const
  {
    ...
    for (KnownNames_t::value_type const& element : known_names)
      if (element.first != DefaultName) elements[element.second.response].insert(element.first);

    size_t nKnownElements = 0;
    if (!elements[rsAccepted].empty()) {
      auto const& selected_elements = elements[rsAccepted];
      Names_t::const_iterator iName = selected_elements.cbegin(), nend = selected_elements.cend();
      out << " accept " << selected_elements.size() << ": '" << *(iName++) << "'";
      while (iName != nend)
        out << ", '" << *(iName++) << "'";
      out << ";";
    ...
    if (!elements[rsRejected].empty()) {
      ...
      Names_t::const_iterator iName = selected_elements.cbegin(), nend = selected_elements.cend();
      out << " reject " << selected_elements.size() << ": '" << *(iName++) << "'";
      while (iName != nend)
        out << ", '" << *(iName++) << "'";
      out << ";";
      nKnownElements += selected_elements.size();
    ...
    if (!elements[rsThrow].empty()) {
      ...
      Names_t::const_iterator iName = selected_elements.cbegin(), nend = selected_elements.cend();
      out << " throw on " << selected_elements.size() << ": '" << *(iName++) << "'";
      while (iName != nend)
        out << ", '" << *(iName++) << "'";
      out << ";";
      nKnownElements += selected_elements.size();
  ...
  NameSelector::Response_t NameSelector::LookupResponse(Name_t name) const
  {
    ...
    return (iResponse == known_names.end()) ? DefaultResponse() : iResponse->second.response;
  ...
  //----------------------------------------------------------------------------
  void NameSelector::InsertItem(KnownNames_t& name_set, Name_t item, Response_t response) const
  {
  ...
  void NameSelector::ProcessItem(KnownNames_t& name_set, Name_t item) const
  {
    ...
    if (iDefinition == definitions.end())
      InsertItem(name_set, item, response);
    else {
  ...
  NameSelector::Definitions_t::const_iterator NameSelector::FindDefinition(Name_t& item) const
  {
    ...
    if ((iDefinition == definitions.end()) && bForceDef) {
      throw art::Exception(art::errors::LogicError) << "no set named '" << item << "'\n";
    }
    return iDefinition;
  ...
  void NameSelector::ClearNameSet(KnownNames_t& name_set) const
  {
    ...
    while (iName != nend) {
      if (iName->first == DefaultName)
        ++iName;
      else
        iName = name_set.erase(iName);
    } // while
  }   // NameSelector::ClearNameSet()

  //----------------------------------------------------------------------------
  NameSelector::Names_t NameSelector::QueriedWithStatus(Response_t answer) const
  {
  ...
  //----------------------------------------------------------------------------
  bool NameSelector::DoCheckQueryRegistry(std::ostream* out /* = nullptr */) const
  {
    Names_t missing;
  ...
  //----------------------------------------------------------------------------
  NameSelector::Response_t NameSelector::ParseMode(Name_t& item,
                                                   Response_t default_mode /* = rsAccepted */)
  {
    if (item[0] == '+') {
      item.erase(0, 1);
      return rsAccepted;
