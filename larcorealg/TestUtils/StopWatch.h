...
namespace testing {
   ...
   *
   */
  template <typename DefaultUnit = std::chrono::duration<double>, // seconds (ratio<1>)
...
testing::StopWatch<DefaultUnit, Clock>::StopWatch(bool start /* = true */)
  : lastStart{start ? now() : TimePoint_t{}}, previousTime{}, isRunning{start}
{}

template <typename DefaultUnit, typename Clock>
template <typename Unit>
testing::StopWatch<DefaultUnit, Clock>::StopWatch(Unit prev, bool start /* = true */)
  : StopWatch(start)
{
  previousTime = prev;
}
...
namespace testing {
  ...
  struct StopWatch<DefaultUnit, Clock>::makeDurationTrait<std::chrono::duration<Rep, Duration>> {
  ...
  struct StopWatch<DefaultUnit, Clock>::makeDurationTrait<std::ratio<Num, Den>> {
    using type = std::chrono::duration<typename StopWatch<DefaultUnit, Clock>::ElapsedTime_t,
                                       std::ratio<Num, Den>>;
