...
void test_values()
{
  ...
  static_assert(
    std::is_same_v<decltype(util::values(data)), std::add_lvalue_reference_t<std::vector<float>>>);
...
void test_const_values()
{
  ...
  //
  // for a vector, we expect exactly the same vector object as return value:
  static_assert(std::is_same_v<decltype(util::const_values(data)),
