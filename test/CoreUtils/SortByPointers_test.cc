...
struct AbsSorter {
  ...
  bool operator()(Data const* a, Data const* b) const { return this->operator()(*a, *b); }
...
void test_SortByPointers()
{
  ...
  BOOST_CHECK_EQUAL_COLLECTIONS(data.cbegin(), data.cend(), sortedData.cbegin(), sortedData.cend());
...
void test_makePointerVector()
{
  ...
  static_assert(std::is_same<std::decay_t<decltype(dataPtr)>, std::vector<int*>>(),
                "Unexpected data type from makePointerVector()");

  BOOST_CHECK_EQUAL_COLLECTIONS(
    dataPtr.cbegin(), dataPtr.cend(), expectedDataPtr.cbegin(), expectedDataPtr.cend());
...
void test_MoveFromPointers()
{
  ...
  std::vector<int*> const dataPtr = {&(data[2]), &(data[3]), &(data[0]), &(data[1])};
  ...
  BOOST_CHECK_EQUAL_COLLECTIONS(
    movedData.cbegin(), movedData.cend(), expectedMovedData.cbegin(), expectedMovedData.cend());
