...
/*
 * Our fixture is GeometryTesterFixture, configured with the object above.
 * It provides:
 * - `geo::GeometryCore const* Geometry()`
 * - `geo::GeometryCore const* GlobalGeometry()` (static member)
 */
using StandardGeometryTestFixture
...
// - BOOST_FIXTURE_TEST_SUITE will provide geometry as a always-present data
//   member in the environment, as "Geometry()"; but a new geometry is
//   initialized on each test case
// - BOOST_GLOBAL_FIXTURE does not provide geometry access, so one has to get
//   geometry as StandardGeometryTestFixture::GlobalGeometry(); on the other
//   hand, geometry is initialized only when a new global feature is explicitly
//   created.
//

// BOOST_FIXTURE_TEST_SUITE(GeometryIterators, StandardGeometryTestFixture)
BOOST_GLOBAL_FIXTURE(StandardGeometryTestFixture)
...
BOOST_AUTO_TEST_CASE( CryostatIteratorsTest )
{
  ...
  geo::GeometryCore const* geom = StandardGeometryTestFixture::GlobalGeometry();
