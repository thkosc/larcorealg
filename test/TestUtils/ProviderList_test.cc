...
struct NormalDatumClass {
  explicit NormalDatumClass(unsigned int v) : value(v) { TrackedMemory.insert(this); }
...
struct UncopiableDatumClass {
  explicit UncopiableDatumClass(const char* v) : value(v) { TrackedMemory.insert(this); }
...
struct UnmovableDatumClass {
  explicit UnmovableDatumClass() { TrackedMemory.insert(this); }
...
namespace testing {
  ...
  {
    BOOST_TEST_MESSAGE(" *** special setup for UnmovableDatumClass *** ");
...
struct PinnedDatumClass : public PinnedDatumClassBase {
  explicit PinnedDatumClass(float x, float y) : value1(x), value2(y) { TrackedMemory.insert(this); }
  ...
  {
    ++nNewCalls;
    return testing::DefaultSetupProviderClass<PinnedDatumClass>::setup(x, y);
  }
...
void TestElement(LIST& l, std::string label = "", bool present = true)
{
  ...
  BOOST_TEST_MESSAGE("Testing class '" << typeid(T).name() << "' label \"" << label << "\" ("
...
void NonConstTest(testing::ProviderList& l)
{
  ...
  // acquire a new one
  BOOST_TEST(l.acquire(std::make_unique<UncopiableDatumClass>("another uncopiable"), "Acquired"));
...
BOOST_AUTO_TEST_CASE(ProviderListTest)
{
  ...
  BOOST_TEST(l->custom_setup<PinnedDatumClass>(&PinnedDatumClass::New, 1.0, 0.0));
